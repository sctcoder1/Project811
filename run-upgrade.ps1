...
Purpose:
  ...
  Schedules a reboot for 7 PM (local time) after setup finishes.
#>
...
# --- Wait for Modern Setup Host to start/finish ---
Log 'Waiting for ModernSetupHost...'
while (-not (Get-Process -Name 'ModernSetupHost' -ErrorAction SilentlyContinue)) { Start-Sleep -Seconds 3 }
Log 'ModernSetupHost detected.'
while (Get-Process -Name 'ModernSetupHost' -ErrorAction SilentlyContinue) { Start-Sleep -Seconds 10 }
Log 'ModernSetupHost exited — staging phase complete.'

# --- Schedule reboot for 7 PM ---
$now = Get-Date
...
schtasks /create /tn 'Win11_DeferredReboot' /sc once /st $time `
    /tr 'shutdown.exe /r /t 60 /c "Windows 11 upgrade will now continue."' `
